...
export function Button({children,className='',type='button',...props}:PropsWithChildren<ButtonHTMLAttributes<HTMLButtonElement>>){return <button type={type} className={`rounded-xl bg-neon px-4 py-2.5 text-sm font-bold text-base transition hover:bg-[#6effa7] disabled:cursor-not-allowed disabled:opacity-50 ${className}`} {...props}>{children}</button>}
