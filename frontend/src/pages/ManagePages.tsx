...
export function BudgetPage() {
  ...
  const createBudget = async (event: FormEvent) => {
    ...
    await financeApi.createBudget({ category_id: Number(category), amount: Number(amount), month: new Date().toISOString().slice(0, 10) })
    setAmount('')
    setCategory('')
    budgets.refetch()
  }
  ...
  const saveBudget = async (budgetId: number, categoryId: number, month: string) => {
    if (!editingAmount || Number(editingAmount) < 0) return
    await financeApi.updateBudget(budgetId, { category_id: categoryId, amount: Number(editingAmount), month })
    setEditingId(null)
    setEditingAmount('')
    budgets.refetch()
  }
  ...
  return <div className="space-y-6"><div><h2 className="text-2xl font-bold">Orçamento</h2><p className="text-muted">Defina limites mensais por categoria.</p></div><Card><form className="mb-6 grid gap-3 sm:grid-cols-[1fr_1fr_auto]" onSubmit={createBudget}><select value={category} onChange={event => setCategory(event.target.value)}><option value="">Selecione a categoria</option>{categories.data?.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><input type="number" min="0" step="0.01" value={amount} onChange={event => setAmount(event.target.value)} placeholder="Valor mensal" /><Button>Definir orçamento</Button></form>{budgets.data?.length ? <div className="space-y-4">{budgets.data.map(budget => { const editing = editingId === budget.id; return <div className="rounded-xl border border-border p-4" key={budget.id}><div className="mb-3 flex flex-wrap items-center justify-between gap-3"><span className="font-medium">{budget.category.name}</span><div className="flex items-center gap-2">{editing ? <><input aria-label={`Novo valor para ${budget.category.name}`} className="!mt-0 w-32" type="number" min="0" step="0.01" value={editingAmount} onChange={event => setEditingAmount(event.target.value)} /><button className="rounded-lg p-2 text-neon hover:bg-panel" onClick={() => saveBudget(budget.id, budget.category_id, budget.month)} aria-label="Salvar orçamento"><Check size={18} /></button><button className="rounded-lg p-2 text-muted hover:bg-panel" onClick={() => { setEditingId(null); setEditingAmount('') }} aria-label="Cancelar edição"><X size={18} /></button></> : <><span>{currency(budget.used)} de {currency(budget.amount)}</span><button className="rounded-lg p-2 text-neon hover:bg-panel" onClick={() => { setEditingId(budget.id); setEditingAmount(String(budget.amount)) }} aria-label={`Editar orçamento de ${budget.category.name}`}><Pencil size={17} /></button><button className="rounded-lg p-2 text-danger hover:bg-panel" onClick={async () => { if (confirm(`Excluir o orçamento de ${budget.category.name}?`)) { await financeApi.deleteBudget(budget.id); budgets.refetch() } }} aria-label={`Excluir orçamento de ${budget.category.name}`}><Trash2 size={17} /></button></>}</div></div><div className="h-2 overflow-hidden rounded-full bg-panel"><div className={budget.status === 'critical' ? 'bg-danger' : budget.status === 'attention' ? 'bg-warning' : 'bg-neon'} style={{ width: `${Math.min(budget.percentage, 100)}%`, height: '100%' }} /></div><div className="mt-2 flex justify-between text-sm text-muted"><span>{budget.percentage}% utilizado</span><span>Restam {currency(budget.remaining)}</span></div></div> })}</div> : <EmptyState message="Defina seu primeiro orçamento mensal." />}</Card></div>
}
